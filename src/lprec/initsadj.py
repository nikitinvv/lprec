...
def create_adj(P, filter_type):
    ...
    fZ = fft.fftshift(fzeta_loop_weights_adj(
    ...
    for k in range(0, P.Nspan):
        z1 = P.aR*(x1[cids]*cos(k*P.beta+P.beta/2)+x2[cids]
                   * sin(k*P.beta+P.beta/2))+(1-P.aR)
        z2 = P.aR*(-x1[cids]*sin(k*P.beta+P.beta/2) +
                   x2[cids]*cos(k*P.beta+P.beta/2))
    ...
    z2n = z2n/P.aR
    lpidsw = where((z1[wids] >= -P.beta/2) &
                   (z1[wids] < P.beta/2) & (abs(z2n) <= 1))
    # left side
    wids2 = ndarray.flatten(
        array(where(log(z2) < log(P.am)-P.g+(P.rhosp[1]-P.rhosp[0]))))
    z2n2 = exp(log(z2[wids2])-log(P.am)+P.g)-(1-P.aR)*cos(z1[wids2])
    z2n2 = z2n2/P.aR
    lpidsw2 = where((z1[wids2] >= -P.beta/2) &
                    (z1[wids2] < P.beta/2) & (abs(z2n2) <= 1))
    lp2p1w = [None]*P.Nspan
    lp2p2w = [None]*P.Nspan
    for k in range(0, P.Nspan):
    ...
    for k in range(0, P.Nspan):
        pids[k] = ndarray.flatten(
            array(where((P.proj >= k*P.beta-P.beta/2) & (P.proj < k*P.beta+P.beta/2))))
    ...
    for k in range(0, P.Nspan):
        lp2p1[k] = (lp2p1[k]-proj0[k])/projl[k] * \
            (size(pids[k])-1)+(proj0[k]-proj0[0])*projp
    ...
    const = (P.N+1)/float32(P.N)*(P.N-1)/2/P.N/sqrt(P.N) * \
        sqrt(P.osangles)*sqrt(P.Nproj)/sqrt(2)
...
def wint(n, t):
    ...
    iv = linalg.inv(exp(transpose(matrix(arange(0, n)))*log(s)))
    u = diff(multiply(exp(transpose(matrix(arange(1, n+2)))*log(s)),
                      transpose(tile(1.0/arange(1, n+2), [n, 1]))))  # integration over short intervals

    W1 = iv*u[range(1, n+1), :]  # x*pn(x) term
    W2 = iv*u[range(0, n), :]  # const*pn(x) term

    # Compensate for overlapping short intervals
    p = 1./concatenate([range(1, n), [(n-1)]*(N-2*(n-1)-1), range(n-1, 0, -1)])
    w = float32(array([0]*N))
    for j in range(0, N-n+1):
        ...
        for k in range(0, n-1):
            w[j+arange(0, n)] = w[j+arange(0, n)] + \
                transpose(W[:, k])*p[j+k]  # % Add to weights
