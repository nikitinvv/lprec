...
class TestLpMethods(unittest.TestCase):
    ...
    def test_foam(self):
        ...
        dif = 0.1
        self.assertTrue(((norm0-602.86017) < dif) & ((norm1-1200.2976) < dif) &
                        (norm2-1139.5126 < dif) & (norm3-444.35742 < dif) & (norm4-926.6201 < dif))

    def test_imp(self):
        [norm0, norm1, norm2, norm3, norm4] = test_imp.test_imp()
        dif = 0.1
        self.assertTrue(((norm0-1993.7756) < dif) & ((norm1-3521.5103) < dif) &
                        (norm2-3454.9438 < dif) & (norm3-2933.0613 < dif) & (norm4-3355.1555 < dif))
